...
public:
  ...
  BT::NodeStatus tick() override
  {
    ...
    aah.sendGoal(goal);
    tables_values = {{ROBOT_TABLE, {robot, {{"ongoing_action", std::string("go_home_") + goal.target_station}}}}};
    ...
    enter_log_file(std::any_cast<std::string>(arg_param["log_file_path"]), "arrive_at_home to" + goal.target_station + " status is " +
                                                                               action_result);
    tables_values = {{ROBOT_TABLE, {robot, {{"ongoing_action", std::string("none")}, {"previous_action", std::string("go_home_") + goal.target_station + "_" + action_result}, {"robot_location", robot_location}}}}};
    set_rdbc_values(std::any_cast<std::string>(arg_param["rdbc_path"]), robot, tables_values);
...
private:
  ...
  std::string robot, robot_location, action_result, robot_number;
